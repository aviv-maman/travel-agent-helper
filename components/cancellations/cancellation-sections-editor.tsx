...
import type { CancelMarkup, Fee, FeeLevel, Localized } from "@/db/schema";
import {
  feeText,
  markupFee,
  newSub,
...
/**
 * The cancellation card body editor, shared by the edit dialog and the create
 * wizard. Owns the category → sub-category → rows navigation; the markup rule
 * and category tree are lifted to the caller (which persists them).
 */
export function CancellationSectionsEditor({
  ...
  rule: CancelMarkup;
  setRule: Dispatch<SetStateAction<CancelMarkup>>;
  cats: EditCategory[];
  setCats: Dispatch<SetStateAction<EditCategory[]>>;
}) {
  ...
  return (
    <div className="flex flex-col gap-4 overflow-y-auto pe-1">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex min-w-[11rem] flex-1 flex-col gap-1">
          <Label className="text-[0.7rem] text-muted-foreground">
            {t("edit.subCategoryName")} <span className="opacity-70">({t("edit.optional")})</span>
          </Label>
          <Input
            dir="rtl"
            className="h-8 text-sm"
            ...
            <div key={ri} className="flex flex-col gap-2 rounded-lg border border-border p-2.5">
              <div className="flex items-start gap-2">
                    ...
                    <div className="relative mt-1.5" dir="ltr">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          else if (row.fee.kind === "amount") setRow(ri, { fee: { ...row.fee, value } });
                        }}
                      />
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Select value={row.level} onValueChange={(v) => setRow(ri, { level: v as FeeLevel })}>
